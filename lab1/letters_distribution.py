...
class LettersDistribution:
    ...
    def make_quiprobable_code_prob_dupl(self):
        quiprobable_code_prob_dupl_list = []
        codes = self.alphabet.code_letter_duplicate.keys()
        for code in codes:
            quiprobable_code_prob_dupl_list.append((code, 1 / len(codes)))
        self.quiprobable_code_prob_dupl = dict(quiprobable_code_prob_dupl_list)

    def make_cyrillic_code_prob_dupl(self, path_to_file: str):
        f = open(path_to_file, encoding='utf-8-sig')
        cyrillic_distribution_dupl_list = []
        for line in f:
            cyrillic_distribution_dupl_list.append(line.split())
            p = float(cyrillic_distribution_dupl_list[-1][-1]) / 100 * 65 / 87
            cyrillic_distribution_dupl_list[-1][-1] = p
        f.close()
        for letter in self.alphabet.letter_code_duplicate.keys():
            if not contains(cyrillic_distribution_dupl_list, letter):
                cyrillic_distribution_dupl_list.append((letter, 1 / 87))
        updated = [(self.alphabet.letter_code_duplicate
                    .get(pair[0]), pair[1]) for pair in cyrillic_distribution_dupl_list]
        self.cyrillic_code_prob_dupl = dict(updated)
