...
def decode(sent_messages: list, code_letter: dict, code_prob: dict, q: float) -> (list, list):
    message_length = len(sent_messages[0])
    code_probs = Queue(message_length)
    for i in range(message_length):
        code_probs.put(code_prob)
    alphas = code_letter.keys()
    real_messages = []
    ...
    for message in sent_messages:
        ...
        for letter in message:
            ...
            real_message.append(code_letter.get(max(local_posterior_distribution, key=itemgetter(1))[0]))
...
def independent_prob_distribution(sent_messages: list, code_letter: dict, code_prob: dict, q: float) -> list:
    alphas = code_letter.keys()
    posterior_distributions = []
    for message in sent_messages:
        posterior_distributions.append([])
